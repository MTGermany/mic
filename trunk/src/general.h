...
// hyperbole-like smooth maximum/minimum function. 
// The additional "max(max(x1,x2), ...) due to artefacts at extreme values:
// maxsmooth(0.01,1e25,0.01) would result in 0, otherwise (-(1.e25-0.01)+1.e25=0)
inline double maxsmooth (double x1, double x2, double dx){
  return max(max(x1,x2), 0.5*(x1+x2) + sqrt(0.25*(x1-x2)*(x1-x2) + dx*dx));
};

inline double minsmooth(double x1, double x2, double dx){
  return min(min(x1,x2),0.5*(x1+x2) - sqrt(0.25*(x1-x2)*(x1-x2) + dx*dx));
};
